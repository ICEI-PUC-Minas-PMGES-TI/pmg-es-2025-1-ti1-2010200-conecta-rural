  ...
  fetch('produtos.json')
    ...
    .then(data => {
      ...
      if (produto) {
        ...
        document.getElementById('contato-produtor').textContent = produto.produtor.propriedade; // Usando 'propriedade' como contato por falta de 'contato' no JSON
        ...
        produto.avaliacoes.forEach(avaliacao => {
          ...
          li.innerHTML = `
            <div class="avaliacao">
              ...
              <p class="nota">Nota: ${'⭐'.repeat(avaliacao.nota)}</p>
            </div>
          `;
